...
public class RentalSystem {
    ...
    private boolean isCarAvailable(Car car, LocalDate startDate, LocalDate endDate) {
        for (Reservation reservation : reservations.values()) {
            if (reservation.getCar().equals(car)) {
                if (startDate.isBefore(reservation.getEnd_date()) && endDate.isAfter(reservation.getStart_date())) {
                    return false;
                }
            }
        }
    ...
    public boolean processPayment(Reservation reservation) {
        return paymentProcessor.processPayment(reservation.getTotal_price());
    }
